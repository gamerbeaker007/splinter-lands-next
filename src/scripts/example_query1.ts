...
async function fetch_all_joined() {
  ...
  logger.info(`📦 Joined ${result.length} deeds`);
  logger.info(`⏱️ Fetch time: ${(end - start) / 1000}s`);
  return { start, end };
}
...
async function fetch_worksites() {
  ...
  logger.info(`📦 ${result.length} worksites`);
  logger.info(`⏱️ Fetch time: ${(end - start) / 1000}s`);
}
...
async function fetch_worksites_including_player() {
  ...
  logger.info(`📦 ${result.length} worksites`);
  logger.info(`fist result ${JSON.stringify(result[1])}`);
  logger.info(`⏱️ Fetch time: ${(end - start) / 1000}s`);
}
