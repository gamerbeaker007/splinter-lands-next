import {fetchResourceSupply} from "@/lib/api/spl/splBaseAPI";
...
export async function computeAndStoreTotalSupply(today: Date) {
    ...
    const results = await Promise.all(
        LEADERBOARD_RESOURCES.map(async (resource) => {
            ...
            const totalSupply = rows.reduce((acc, row) => acc + Number(row.amount || 0), 0);
    ...
    console.log(`✅ Stored resource supply for ${today.toISOString().split('T')[0]}`);
}
