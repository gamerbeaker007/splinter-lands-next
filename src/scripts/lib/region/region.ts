...
async function createManyInChunks<T>(
    label: string,
    allItems: T[],
    createManyFn: (args: { data: T[]; skipDuplicates?: boolean }) => Promise<Prisma.BatchPayload>,
    chunkSize = 2000
) {
    ...
    logger.info(`✅ Finished inserting ${label}. Total inserted (non-duplicates): ${totalInserted}`);
}
