"use client";

import { lookupTransaction } from "@/lib/backend/actions/land-manager/overview-actions";
import { refreshCardCollection } from "@/lib/backend/actions/land-manager/renew-rental-actions";
import {
  broadcastOperations,
  KeychainKeyTypes,
  waitForTransactions,
} from "@/lib/frontend/splBroadcast";
import { buildUnstakeWorkersOp } from "@/lib/shared/operations/opBuilders";
import { useCallback, useState } from "react";

interface Params {
  username: string;
  onSuccess?: () => void;
}

export interface UseUnstakeWorkerAction {
  busy: boolean;
  error: string | null;
  clearError: () => void;
  execute: (cardUid: string, deedUid: string) => Promise<void>;
}

export function useUnstakeWorkerAction({
  username,
  onSuccess,
}: Params): UseUnstakeWorkerAction {
  const [busy, setBusy] = useState(false);
  const [error, setError] = useState<string | null>(null);

  const execute = useCallback(
    async (cardUid: string, deedUid: string) => {
      if (!username) {
        setError("Not logged in.");
        return;
      }
      setBusy(true);
      setError(null);
      try {
        const op = buildUnstakeWorkersOp(username, deedUid, [cardUid]);
        const result = await broadcastOperations(
          username,
          [op],
          KeychainKeyTypes.posting
        );
        if (!result.success) {
          setError(result.error ?? "Broadcast failed");
          return;
        }
        await waitForTransactions(result.txIds, lookupTransaction);
        await refreshCardCollection();
        onSuccess?.();
      } catch (err) {
        setError(err instanceof Error ? err.message : "Unstake failed");
      } finally {
        setBusy(false);
      }
    },
    [username, onSuccess]
  );

  return {
    busy,
    error,
    clearError: () => setError(null),
    execute,
  };
}
