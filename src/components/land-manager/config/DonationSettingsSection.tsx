...
export default function DonationSettingsSection({
  donation,
  onChange,
  layout = "accordion",
}: Props) {
  ...
  const content = (
    <>
      <Typography
        ...
        mb={1}
      >
        Configure optional donation transfers after harvest. Set a daily cap per
        resource to limit total donations on a single day across all regions.
      </Typography>
      ...
      <Paper variant="outlined" sx={{ mt: 1.5, p: 1.5, borderRadius: 1 }}>
        ...
        <Stack spacing={0.75} sx={{ pl: 2.5 }}>
          {(
            [
              <>
                <strong>10% land tax</strong> (Castles &amp; Keeps) is deducted
                by the game before harvest — donation % is applied to the
                post-tax harvestable amount.
              </>,
              <>
                <strong>Daily cap is cumulative across all regions</strong>. If
                Region A uses 38,000 of a 40,000 GRAIN cap, Region B&apos;s
                donation is trimmed to the remaining 2,000 — not dropped
                entirely.
              </>,
              <>
                Donations below <strong>5 units</strong> per resource per region
                are automatically skipped as too small to be worth the
                transaction cost.
              </>,
            ] as React.ReactNode[]
          ).map((text, i) => (
