...
export default function RentEmptyWorkersSection({
  rental,
  onChange,
  layout = "accordion",
}: Props) {
  ...
  const content = (
    <>
      <Typography
        ...
        mb={1}
      >
        Strategy and spending caps for renting cards into empty worker slots on
        powered plots. Use 0 to disable a cap.
      </Typography>
      ...
      <FormControl component="fieldset" sx={{ mb: 2 }}>
        ...
        >
          {(
            Object.entries(RENTAL_STRATEGY_LABELS) as [RentalStrategy, string][]
          ).map(([value, label]) => (
            <FormControlLabel
              key={value}
              value={value}
      ...
      <Stack gap={0.5} mb={2}>
        ...
        <Alert severity="info" sx={{ py: 0.5 }}>
          <Typography variant="caption">
            ...
            re-fetches live market data, so running 10 slots at a time lets you
            catch fresher listings between batches. Larger batches process more
            slots at once but market prices may shift during the run — the first
            slots get the best picks; later slots see what&apos;s left after
            earlier selections.
          </Typography>
        </Alert>
      </Stack>
      ...
      >
        <Stack>
          ...
          <Typography variant="caption" color="text.secondary">
            When enabled, the Renew Rentals flow skips cards that are rented but
            not currently staked on a land plot. Useful if you rent cards for
            multiple purposes and only want to renew those actively working.
          </Typography>
        </Stack>
        <Switch
