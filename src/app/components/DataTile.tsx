...
export default function DataTile() {
    const [activeAll, setActiveAll] = useState<ActiveDto[]>([]);
        ...
        fetch('/api/active')
            ...
            .then(data => {
                if (Array.isArray(data)) {
                    ...
                    console.error('Invalid activeAll response:', data);
                    setActiveAll([]); // or show error
                }
            })
            .catch(err => {
                console.error(err);
    ...
    return (
        <>
            <div className="card bg-base-100 shadow-md p-4">
                <h2 className="text-lg font-bold mb-2">Active Data</h2>
                <ul className="list-disc list-inside">
                    {activeAll.map((item, idx) => (
                        <li key={idx}>{((item.activeBasedOnPp ?? 0) / MAX_PLOTS * 100).toFixed(1)}%</li>
                    ))}
                </ul>
            </div>
            <div className="card bg-base-100 shadow-md p-4">
